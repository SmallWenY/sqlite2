...
public class BillFragment extends Fragment {
    ...
    private void initData() {
        ...
        if (cursor1.moveToFirst()) {
            do {
                cal_can1 += cursor1.getInt(cursor1.getColumnIndex("dish_price"));
            } while (cursor1.moveToNext());
        }

        if (cursor2.moveToFirst()) {
            do {
                cal_can2 += cursor2.getInt(cursor2.getColumnIndex("dish_price"));
            } while (cursor2.moveToNext());
        }

        if (cursor3.moveToFirst()) {
            do {
                cal_can3 += cursor3.getInt(cursor3.getColumnIndex("dish_price"));
    ...
    private void  initPieData(){
        ...
        mDataSet.setColors(Color.rgb(60,179,113),Color.rgb(255,69,0),Color.rgb(255,185,15));
